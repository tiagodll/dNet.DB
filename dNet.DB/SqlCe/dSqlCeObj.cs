...
namespace dNet.DB.SqlCe
{
    public class dSqlCeObj : IDbObj
	{
        ...
        {
            List<string> PKColumn = obj.GetPKColumns();
            bool resp = false;
            string query = "SELECT * FROM " + obj.GetTableName() + " WHERE 1=1 ";
            if (where.Length > 0)
                query += " AND " + where;
			Hashtable htemp = new Hashtable();
            Hashtable PKValue = obj.GetPKs();
            foreach(string key in PKValue.Keys)
            {
                query += " AND " + key + "=@" + key;
                htemp.Add(key, PKValue[key]);
            }
            ...
            dSqlCeUtil.AddHashParameters(cmd, htemp);
        ...
        public bool Save(dModel obj)
        {
            ...
            foreach(string key in pks.Keys)
                query += " AND " + key + "=" + pks[key];

            if (pks.Count==0 || TryConvert.ToInt(new dSqlCeSql() { ConnectionString=this.ConnectionString }.SelectScalar(query)) == 0)
                return Insert(obj);
            else
                return Update(obj);
        }
		...
		public bool Insert(dModel obj)
		{
                    ...
                    if (!(attributes[0] as dNet.DB.ColumnAttribute).IsAutoIncrement)
                    {
                        ...
                        if (TryConvert.ToBool(pinfo.GetValue(obj, null)))
                        {
                            ...
                            if (pks.ContainsKey(column) && pks[column] == null)
                                pi.SetValue(obj, TryConvert.ToDecimal(new SqlCeCommand("SELECT MAX(" + pks[column] + ") AS N FROM " + obj.GetTableName(), dc).ExecuteScalar()) + 1, null);
            ...
            SqlCeCommand cmd = new SqlCeCommand(sb1.ToString(), dc);
            dSqlCeUtil.AddHashParameters(cmd, htemp);
			decimal n = TryConvert.ToDecimal(cmd.ExecuteScalar(), 0);
			dc.Close();
			return (n > 0);
		}
		#endregion
